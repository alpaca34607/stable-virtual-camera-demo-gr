...
class SevaRenderer(object):
        ...
        if isinstance(input_img_path_or_tuples, str):
            ...
            for img, K in zip(input_imgs, input_Ks):
                ...
                img, K = transform_img_and_K(
                    img, shorter, K=K[None], size_stride=64)
                assert isinstance(K, torch.Tensor)
                K = K / K.new_tensor([img.shape[-1],
                                     img.shape[-2], 1])[:, None]
    ...
    def export_output_data(self, preprocessed: dict, output_dir: str):
        ...
        target_c2ws, target_Ks = self.get_target_c2ws_and_Ks_from_gui(
            preprocessed)
        ...
        img_whs = np.array(input_wh)[None].repeat(
            len(input_imgs) + len(target_Ks), 0)
        ...
        create_transforms_simple(
            output_dir, img_paths, img_whs, all_c2ws, all_Ks)
        split_dict = {
    ...
    def render(
        ...
        target_indices = np.arange(
            num_inputs, num_inputs + num_targets).tolist()
        # Get anchor cameras.
        T = VERSION_DICT["T"]
        ...
        all_imgs_np = (
            F.pad(input_imgs, (0, 0, 0, 0, 0, 0, 0,
                  num_targets), value=0.0).numpy()
            * 255.0
        ).astype(np.uint8)
        image_cond = {
            "img": all_imgs_np,
        ...
        gt_input_inds = [anchor_argsort.index(
            i) for i in range(input_c2ws.shape[0])]
...
def setup_tunnel(
    ...
    tunnel = Tunnel(remote_host, remote_port, local_host,
                    local_port, share_token, share_server_tls_certificate)
...
def start_server_and_abort_event(request: gr.Request):
    try:
        ...
        renderer_obj = SevaRenderer(server)

        print(f"Renderer created: {renderer_obj}")
        ...
        print(f"Error in start_server_and_abort_event: {e}")
        import traceback
        traceback.print_exc()
        return None, gr.HTML("Failed to start server.", container=True), request.session_hash
    server = viser.ViserServer()
        ...
        with gr.Tabs():
            with gr.Tab("Basic"):
                render_btn = gr.Button(
                    "Render video", interactive=False, render=False)
                with gr.Row():
                    with gr.Column():
                        with gr.Group():
                        ...
                        with gr.Group():
                            ...
                            preprocess_btn.click(
                                lambda r, *args: (
                                    [*r.preprocess(*args), gr.update(interactive=True)] if r is not None
                                    else [gr.update(), gr.update(), gr.update(), gr.update(interactive=True)]
                                ),
                        ...
                        with gr.Row():
                            ...
                            cfg = gr.Slider(1.0, 7.0, value=4.0,
                                            label="CFG value")
                        with gr.Row():
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        with gr.Row():
                            seed = gr.Number(value=23, label="Random seed")
                            chunk_strategy.render()
                            cfg = gr.Slider(1.0, 7.0, value=3.0,
                                            label="CFG value")
                        with gr.Row():
                            camera_scale = gr.Slider(
                                0.1,
